...
from utils.layers import Flatten, VariationalLayer, VariationalDecoderOutput, ResNetBlock1d
...
parser.add_argument("--nfilters", type=str, default="64-128-128", help="number of filters for convolutional layers")
...
classifier = nn.Sequential(
    nn.Conv1d(in_channels=xdim[0], out_channels=n_filters[0], kernel_size=7, padding=3),
    nn.BatchNorm1d(n_filters[0]),
    get_activation(options.activation)(),
	ResNetBlock1d(in_channels=n_filters[0], out_channels=n_filters[1], kernel_size=3, layers=2, activation=get_activation(options.activation)),
	ResNetBlock1d(in_channels=n_filters[1], out_channels=n_filters[2], kernel_size=3, layers=2, activation=get_activation(options.activation)),
	ResNetBlock1d(in_channels=n_filters[2], out_channels=n_filters[3], kernel_size=3, layers=2, activation=get_activation(options.activation)),
    #nn.MaxPool1d(2),
    ResNetBlock1d(in_channels=n_filters[3], out_channels=n_filters[4], kernel_size=3, layers=2, activation=get_activation(options.activation)),
...
model_name = f"SSVAE-ResNet-simple_lr={options.lr}_bs={options.batch_size}_nfilters={options.nfilters}_zdim={options.zdim}_activation={options.activation}_scheduler={options.scheduler}"
